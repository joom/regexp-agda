...
module ExtrinsicHOF where
  ...
  -- Matching algorithm
  match : StdRegExp → (s : List Char) → ((s' : List Char) → Suffix s' s → Bool) → Acc Suffix s → Bool
  match ∅ˢ s k _ = false
  match (Litˢ _) [] _ _ = false
  match (Litˢ c) (x ∷ xs) k _ = (x == c) ∧ (k xs Stop)
  match (r₁ ·ˢ r₂) s k (acc f) = match r₁ s (λ s' sf → match r₂ s' (λ s'' sf' → k s'' (suffix-trans sf' sf)) (f s' sf)) (acc f)
  match (r₁ ⊕ˢ r₂) s k perm = (match r₁ s k perm) ∨ (match r₂ s k perm)
  match (r ⁺ˢ) s k (acc f) = (match r s k (acc f)) ∨ (match r s (λ s' sf → match (r ⁺ˢ) s' (λ s'' sf' → k s'' (suffix-trans sf' sf)) (f s' sf)) (acc f))
  ...
  match-soundness (r₁ ·ˢ r₂) s k (acc f) m with match-soundness r₁ s (λ s' sf → match r₂ s' (λ s'' sf' → k s'' (suffix-trans sf' sf)) (f s' sf)) (acc f) m
  match-soundness (r₁ ·ˢ r₂) s k (acc f) m | (xs , ys , r) , a , b , c
    with match-soundness r₂ ys (λ s' sf → k s' (suffix-trans sf r)) (f ys r) c
  match-soundness (r₁ ·ˢ r₂) .(xs ++ as ++ bs) k (acc f) m | (xs , .(as ++ bs) , r) , refl , b , c | (as , bs , r1) , refl , b1 , c1 =
  ...
  match-soundness (r₁ ⊕ˢ r₂) s k perm m | inj₂ x with match-soundness r₂ s k perm x
  match-soundness (r₁ ⊕ˢ r₂) s k perm m | inj₂ x | (p , q , r) , a , b , c = (p , (q , r)) , (a , (∈ˢ⊕₂ b , c))
  match-soundness (r ⁺ˢ) s k (acc f) m
    with or-eq {match r s k (acc f)} { match r s (λ s' sf → match (r ⁺ˢ) s' (λ s'' sf' → k s'' (suffix-trans sf' sf)) (f s' sf)) (acc f)} m
  match-soundness (r ⁺ˢ) s k (acc f) m | inj₁ x with match-soundness r s k (acc f) x
  match-soundness (r ⁺ˢ) s k (acc f) m | inj₁ x | (xs , ys , sf) , a , fst , snd = (xs , (ys , sf)) , (a , (∈ˢS+ fst , snd))
  match-soundness (r ⁺ˢ) s k (acc f) m | inj₂ x with match-soundness r s (λ s' sf → match (r ⁺ˢ) s' _ (f s' sf)) (acc f) x
  match-soundness (r ⁺ˢ) s k (acc f) m | inj₂ x | (xs , (ys , sf)) , eq , xs∈rS , d with match-soundness (r ⁺ˢ) ys (λ s' sf' → k s' (suffix-trans sf' sf)) (f ys sf) d
  match-soundness (r ⁺ˢ) s k (acc f) m | inj₂ x | (xs , (ys , sf)) , eq , xs∈rS , d | (ys' , ys'' , sf') , eq1 , ys'∈rP , d1 with sym (append-assoc xs ys' ys'')
  match-soundness (r ⁺ˢ) .(xs ++ ys' ++ ys'') k (acc f) m | inj₂ x | (xs , .(ys' ++ ys'') , sf) , refl , xs∈rS , d | (ys' , ys'' , sf') , refl , ys'∈rP , d1 | app =
  ...
  match-completeness (r₁ ·ˢ r₂) .(xs ++ ys) k (acc f) ((xs , ys , sf) , refl , ∈ˢ· {_}{ms}{ns} eq' inL inL' , pf) with eq' | append-assoc ms ns ys
  match-completeness (r₁ ·ˢ r₂) .((ms ++ ns) ++ ys) k (acc f) ((.(ms ++ ns) , ys , sf) , refl , ∈ˢ· {.(ms ++ ns)} {ms} {ns} eq' inL inL' , pf) | refl | b
  ...
  ... | x = match-completeness r₁ ((ms ++ ns) ++ ys) _ (acc f) (((ms , ns ++ ys , t) , b  , inL , x))
  match-completeness (r₁ ⊕ˢ r₂) s k perm ((xs , ys) , b , ∈ˢ⊕₁ c , d) = either-if (inj₁ (match-completeness r₁ s k perm ((xs , ys) , b , c , d) ))
  match-completeness (r₁ ⊕ˢ r₂) s k perm ((xs , ys) , b , ∈ˢ⊕₂ c , d) = either-if (inj₂ (match-completeness r₂ s k perm ((xs , ys) , b , c , d)))
  match-completeness (r ⁺ˢ) s k (acc f) ((xs , ys , sf) , b , ∈ˢS+ x , d) = either-if (inj₁ (match-completeness r s k (acc f) ((xs , (ys , sf)) , b , x , d)))
  match-completeness (r ⁺ˢ) s k (acc f) ((._ , ys , sf) , b , ∈ˢC+ {.(s₁ ++ s₂)}{s₁}{s₂} refl q c , d) with match r s k (acc f)
  match-completeness (r ⁺ˢ) s k (acc f) ((._ , ys , sf) , b , ∈ˢC+ refl q c , d) | true = refl
  match-completeness (r ⁺ˢ) s k (acc f) ((._ , ys , sf) , b , ∈ˢC+ {.(s₁ ++ s₂)}{s₁}{s₂} refl q c , d) | false
  ...
  match-completeness (r ⁺ˢ) ._ k (acc f) ((._ , ys , sf) , refl , ∈ˢC+ {.(s₁ ++ s₂)}{s₁}{s₂} refl q c , d) | false | t | x =
    match-completeness r ((s₁ ++ s₂) ++ ys) _ (acc f) ((s₁ , s₂ ++ ys , t) , append-assoc s₁ s₂ ys , q , x)
